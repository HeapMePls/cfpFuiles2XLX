...
class PatientHistory {
    ...
    // Eliminar el campo de paciente existente si ya está inicializado
    if (this.patient_control) {
        this.patient_control.$input.remove();
        this.patient_control = null;
    }
    ...
    // Guardar la referencia al control del paciente
    this.patient_control = patient;
    ...
    if (frappe.route_options && !this.patient_id) {
        ...
        patient.set_value('');
        this.patient_id = '';
        me.page.main.find('.patient-documents').html("")
      }
    ...
    me.page.main.on("click", "#printButton", function() {
      const content = me.page.main[0].innerHTML;

      const htmlContent = '<html><body>' + content + '</body></html>';

      frappe.require("assets/frappe/js/lib/jspdf.min.js", function() {
          const pdf = new jsPDF();
          pdf.fromHTML(htmlContent, 15, 15, { 'width': 170 });
        ...
        this.page.add_inner_button(__('Imprimir'), function () {
          window.print(); // Aquí llamamos a la función de impresión del navegador
      });
  }
